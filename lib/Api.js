...
class Api {
    ...
    async addPlugin (pluginInfo, installOptions) {
        ...
        if (!pluginInfo) {
            return Promise.reject(
                new Error('The parameter is incorrect. The first parameter should be valid PluginInfo instance')
            );
        }
        ...
        pluginInfo.getFilesAndFrameworks(this.platform)
            ...
            .forEach(item => {
                actions.push(actions.createAction(
                    this._getInstaller(item.itemType, item, pluginInfo.dir, pluginInfo.id, installOptions, projectFile),
                    this._getUninstaller(item.itemType, item, pluginInfo.dir, pluginInfo.id, installOptions, projectFile)
                ));
            });
        ...
        return actions.process(this.platform, this.root)
            .then(() => {
                ...
                if (!installOptions.variables.PACKAGE_NAME) {
                    installOptions.variables.PACKAGE_NAME = this._handler.package_name(this.root);
                }
                ...
                this.#addModulesInfo(pluginInfo, targetDir);
            });
    }

    async removePlugin (plugin, uninstallOptions) {
        ...
        return actions.process(this.platform, this.root)
            .then(() => {
                ...
                this.#removeModulesInfo(plugin, targetDir);
                // Remove stale plugin directory
                // TODO: this should be done by plugin files uninstaller
                fs.rmSync(
                    path.resolve(this.root, 'Plugins', plugin.id),
                    { force: true, recursive: true }
                );
            });
    }

    _getInstaller (type, item, plugin_dir, plugin_id, options, project) {
        const installer = this._handler[type];
        ...
        if (!installer) {
            ...
            const wwwDest = options.usePlatformWww
                ? this.getPlatformInfo().locations.platformWww
                : this._handler.www_dir(this.root);
    ...
    _getUninstaller (type, item, plugin_dir, plugin_id, options, project) {
        const installer = this._handler[type];
        ...
        if (!installer) {
            ...
            if (['asset', 'js-module'].includes(type)) {
     ...
     */
    #addModulesInfo (plugin, targetDir) {
        ...
        const modulesToInstall = plugin.getJsModules(this.platform)
            .filter(moduleToInstall => !installedPaths.includes(moduleToInstall.file))
            .map(moduleToInstall => {
        ...
        this.#writePluginModules(targetDir);
     ...
     *   should be written to.
     */
    #removeModulesInfo (plugin, targetDir) {
        const installedModules = this._platformJson.root.modules || [];
        const modulesToRemove = plugin.getJsModules(this.platform)
            .map(jsModule => ['plugins', plugin.id, jsModule.src].join('/'));

        const updatedModules = installedModules.filter(
            installedModule => !modulesToRemove.includes(installedModule.file)
        );
